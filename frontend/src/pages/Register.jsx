...
export default function Register() {
  ...
  // Live password checklist validation calculations
  ...
  // Compute live strength level
  const criteriaMetCount = Object.values(checks).filter(Boolean).length;
  let strengthLabel = '';
  ...
  if (pass.length > 0) {
    if (criteriaMetCount <= 1) {
      ...
      strengthTextClass = 'text-red-400';
      ...
      <div className="hidden lg:flex lg:w-[60%] flex-col justify-between p-12 relative z-10 border-r border-white/5 bg-gradient-to-b from-transparent via-white/[0.01] to-transparent">
        ...
        <div className="my-auto py-12 flex flex-col items-start gap-8">
          <div className="space-y-4">
            <h1 className="text-5xl md:text-6xl xl:text-7xl font-extrabold text-white tracking-tight leading-none">
              InterviewForge
              ...
              {pass.length > 0 && (
                <div className="mt-1 flex flex-col gap-1.5">
                  <div className="flex justify-between items-center text-[10px]">
                    ...
                    <span className={`font-semibold capitalize ${strengthTextClass}`}>{strengthLabel}</span>
                  </div>
                    ...
                    {[1, 2, 3, 4].map((stepIndex) => (
                      <div
                        ...
                        className={`flex-1 rounded-full transition-all duration-300 ${
                          criteriaMetCount >= stepIndex ? strengthColor : 'bg-slate-800'
                        }`}
                      />
                    ))}
                  </div>
                  ...
                  <div className="mt-1.5 p-2.5 rounded-lg bg-white/[0.01] border border-white/5 space-y-1 text-[11px] text-slate-400">
                    <div className="flex items-center gap-1.5">
                      <div className={`w-3.5 h-3.5 rounded-full flex items-center justify-center border transition-all ${checks.length ? 'border-emerald-500 bg-emerald-500/10 text-emerald-400' : 'border-slate-800 text-slate-600'}`}>
                        <Check className="w-2 h-2" />
                      </div>
                      <span>At least 8 characters</span>
                    </div>
                    <div className="flex items-center gap-1.5">
                      <div className={`w-3.5 h-3.5 rounded-full flex items-center justify-center border transition-all ${checks.upper ? 'border-emerald-500 bg-emerald-500/10 text-emerald-400' : 'border-slate-800 text-slate-600'}`}>
                        <Check className="w-2 h-2" />
                      </div>
                      <span>One uppercase letter</span>
                    </div>
                    <div className="flex items-center gap-1.5">
                      <div className={`w-3.5 h-3.5 rounded-full flex items-center justify-center border transition-all ${checks.number ? 'border-emerald-500 bg-emerald-500/10 text-emerald-400' : 'border-slate-800 text-slate-600'}`}>
                        <Check className="w-2 h-2" />
                      </div>
                      <span>One number</span>
                    </div>
                    <div className="flex items-center gap-1.5">
                      <div className={`w-3.5 h-3.5 rounded-full flex items-center justify-center border transition-all ${checks.special ? 'border-emerald-500 bg-emerald-500/10 text-emerald-400' : 'border-slate-800 text-slate-600'}`}>
                        <Check className="w-2 h-2" />
                      </div>
